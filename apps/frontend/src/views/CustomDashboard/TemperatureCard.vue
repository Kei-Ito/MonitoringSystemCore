<template>
  <div class="container" :class="{ active: isSampling,error:isError }">
    ...
    <div class="content">
      ...
      <span class="content-text  font-weight-bold fs-3 px-2 p-0" :class="{error:isError}" style="width:30px">{{ channelSetting?.unit }}</span>
    </div>
  </div>
</template>
...
.container.active {
  box-shadow: 0 0 10px 3px rgba(235, 54, 9, 0.4);
}
...
.title.active {
  background: linear-gradient(87deg,
        ...
        rgba(153,102,204, 0.7) 100%) !important;
}
