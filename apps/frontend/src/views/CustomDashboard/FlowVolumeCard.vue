<template>
  <div class="container" :class="{ active: isSampling,error:isError }">
    ...
    <div class="content">
      ...
      <span class="content-text  font-weight-bold fs-2 px-3  p-0" :class="{error:isError}">{{ roundValue(props.value, channelSetting?.decimals ?
        channelSetting.decimals :0) }}</span>
      <span class="content-text  font-weight-bold fs-3 p-0"  :class="{error:isError}">{{ "L/min" }}</span>
    </div>
  </div>
</template>
