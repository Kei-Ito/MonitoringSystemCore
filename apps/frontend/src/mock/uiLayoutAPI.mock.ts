...
function getDashboardLayout() {
  ...
  chart1.channel_uuids = ['channel_mock_uuid0', 'channel_mock_uuid1', 'channel_mock_uuid2', 'channel_mock_uuid3', 'channel_mock_uuid4', 'channel_mock_uuid5', 'channel_mock_uuid6', 'channel_mock_uuid7', ];
  chart1.grid_layout = {
    ...chart1.grid_layout,
    h: 8,
    w: 12,
  ...
  chart2.channel_uuids = ['channel_mock_uuid0', 'channel_mock_uuid1', 'channel_mock_uuid2', 'channel_mock_uuid3', 'channel_mock_uuid4', 'channel_mock_uuid5', 'channel_mock_uuid6', 'channel_mock_uuid7', ];
  chart2.grid_layout = {
    ...chart2.grid_layout,
    h: 8,
    w: 12,
...
function getTrendLayout() {
  ...
  const trendLayout:ChartConfig[] = []
  const chart1 = createChartForInitialization(ChartTypes.LineChart);
  chart1.grid_layout.h = 8;
  chart1.category1 = '照射炉1';
  chart1.category2 = '液温';
  chart1.chart_title = 'super special very very long text';
  ...
  const category2List = ['液温','UV強度','炉内温度','ランプ電圧','ランプ電流','安定器電流','冷却ファン周波数',null]; 
  trendLayout.push(chart1);
  
  for ( let i = 0; i < 8; i++) {
    let chart = createChartForInitialization(ChartTypes.LineChart);
    chart = {
      ...chart,
      chart_title: `title ${i}`,
      chart_unit : 'unit',
